...
const LoginModal = ({ isOpen, onClose, onSwitchToRegister, onLoginSuccess }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const result = loginUser(formData.emailOrUsername, formData.password);
    
    if (result.success) {
      alert("✅ " + result.message);
      onLoginSuccess(result.user); // Callback để update Header
      onClose();
    } else {
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-pink-400 text-sm"
            />
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-4 py-3 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-pink-400 text-sm pr-12"
              />
              <button
                type="button"
